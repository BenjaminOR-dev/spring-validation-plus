...
Spring Validation Plus adiciona mais de **85 anotações** (`@Required`, `@Same`, `@RequiredIf`, …) que funcionam como `@NotNull` ou `@Size`: você as coloca nos seus DTOs e elas são executadas com `@Valid` ou `@Validated`. Inclui mensagens i18n (es/en/pt), tratamento unificado de erros JSON e suporte opcional para regras de banco de dados (`@Unique`, `@Exists`).
...
| Sem matching de campos nem `required_with` nativos | `@Same("password")`, `@RequiredWith("password")` no campo |
...
**Quando o Validation Plus compensa:** campos que coincidem (`@Same`), updates parciais (`@Nullable`), unicidade em BD (`@Unique`), regras condicionais, respostas de erro prontas para o frontend, i18n desde o primeiro dia, ou vindo do Laravel/PHP.
...
|-------------|------------------------------|-----------------|
| 3.x | 8.x | Suportado (CI padrão: `verify`) |
| 4.x | 9.x | Suportado (CI: `verify-hv9` → `mvn verify -Phv9`) |

O mesmo JAR do starter serve nos dois. Use **≥ 0.3.7**. A ordem de auto-config (`beforeName` / `afterName`) e a interpolação de `{field}` foram feitas para Boot 3 e Boot 4.

> Não há perfil `-Pboot4` de BOM aqui (diferente de fluent-query / fluent-map). A compatibilidade com Boot 4 é verificada compilando e testando contra **Hibernate Validator 9.x** com `-Phv9`.
...
**JPA não vem incluído** no Spring Boot nem no Validation Plus. Se você só usa regras de formato, tipos ou entre campos (`@Required`, `@Same`, …), não precisa instalar nada mais.
...
Os placeholders `{field}` e `{other}` são resolvidos pelo interpolador incluído (`ValidationPlusMessageInterpolator`). Em regras condicionais no nível do campo (`@RequiredIf`, `@ProhibitedIf`, …), `{field}` é a propriedade anotada que falhou e `{other}` o campo observado. Se você os ver sem substituir, o nome do campo vazio, ou o mesmo nome repetido dos dois lados (corrigido na **0.3.7**), use o starter **≥ 0.3.7** e verifique que não definiu um `LocalValidatorFactoryBean` custom sem esse interpolador.
...
Consulte **[spring-validation-plus-example/README.pt.md](spring-validation-plus-example/README.pt.md)** — inclui:

- Mapa **DTO → padrão → endpoint**
- Exemplos de `@Unique`, `@ModelAttribute`, `@Valid` aninhado, `@RequiredIf`, `@Same`
...
| `@MustBe` | Valor exatamente igual a uma constante (ex. `@MustBe("T")`) |
...
└── spring-validation-plus-example/    ← referência executável (ver README.pt.md)
```

| Job CI | Comando | Significado |
|--------|---------|-------------|
| `verify` | `mvn clean verify` | Stack padrão (Boot 3.x / HV 8.x) |
| `verify-hv9` | `mvn clean verify -Phv9` | Compatibilidade Boot 4 (HV 9.x) |

```bash
# Compilar e executar todos os testes (HV 8.x por padrão)
docker compose run --rm maven

# Compatibilidade Boot 4 (Hibernate Validator 9.x)
